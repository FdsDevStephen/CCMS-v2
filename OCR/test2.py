...
PDF_PATH = Path(
    r"C:\Users\steph\OneDrive\Desktop\CEG\CCMS v2\uploads\WP-101673-2022-D.pdf"
)
...
# Only the first 30 pages are scanned for discovery.
SEARCH_PAGES = 30
...
#
# Supports:
# SYNOPSIS + DATE + EVENTS
# SYNOPSIS + DATE + DESCRIPTIONS
# SYNOPSIS + DATE + PARTICULARS
# continuation pages with only:
# DATE + EVENTS
# ==========================================================
...
def collect_candidates(
    ...
    for page, text in page_text.items():
        ...
        table_score = synopsis_table_score(
            text
        )
...
def select_synopsis(
    ...
    for candidate in candidates:
        ...
        if candidate.page > synopsis_page_limit:
            continue
        ...
        if candidate.kind == "heading":
            ...
            # "List of dates and Synopsis" can be
            # an Index/Contents entry.
            if (
                "LIST OF DATES"
                in line_normalized
            ):
    ...
    # Prefer standalone Synopsis heading.
    standalone_headings = [
        c
        for c in valid
        if (
...
def select_brief_facts(
    ...
    # Brief Facts can begin on the same physical page
    # as the Synopsis.
    same_page = [
        c
        for c in valid
...
# FIND SYNOPSIS END
#
# Synopsis stops when Brief Facts begins.
# ==========================================================
...
def find_synopsis_end(
    ...
    for page in range(
        ...
        for line in lines:
            ...
            if (
                is_brief_facts_heading(
                    line
                )
                or is_grounds_heading(
                    line
                )
                or is_prayer_heading(
                    line
                )
                or is_new_document(
                    line
...
#
# CRITICAL:
# Brief Facts is allowed to continue across pages.
# The START page is never considered an ending page.
# ==========================================================
...
def find_brief_facts_end(
    ...
    for page in range(
        ...
        for index, line in enumerate(
            ...
            # DO NOT stop merely because OCR produced a
            # signature-like line. Require a real advocate/footer
            # marker so the facts can continue safely.
            if (
            ...
            # Date is a boundary only when an advocate marker
            # follows nearby. A date inside the facts is valid text.
            if normalized.startswith("DATE"):

                nearby = " ".join(
                    normalize(x)
                    for x in lines[
                        index:min(index + 5, len(lines))
                    ]
                )
...
#
# CRITICAL:
# Prayer is allowed to continue across pages.
# The start page is never considered an ending page.
# ==========================================================
...
#
# Supports table continuation pages where SYNOPSIS
# is not present on the same page.
# ==========================================================
...
def find_table_synopsis_start(
    ...
    if (
        ...
        if date_rows >= 2:
            ...
            # OCR may separate Date and Events.
            for index, line in enumerate(
                lines
            ):
...
def find_section_start(
    ...
    if section == "synopsis":
        ...
        # For a selected table candidate, use the entire
        # selected OCR page as the starting content.
...
def is_brief_facts_footer(
    ...
    # --------------------------------------------------
    # A Brief Facts section can contain dates and other
    # short lines as part of the actual facts. Therefore
    # DO NOT stop on a standalone DATE or OCR signature.
    # --------------------------------------------------

    # Explicit Place/footer marker.
    ...
    # Date is a footer only when an advocate/signature
    # marker occurs immediately after it.
    if normalized.startswith("DATE"):
    ...
    # Common location footer, but only near the bottom
    # of the page.
    if (
        index >= max(0, len(lines) - 6)
        and normalized in {
            "BENGALURU",
            "BANGALORE",
            "DHARWAD",
